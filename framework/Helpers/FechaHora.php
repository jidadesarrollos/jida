...
class FechaHora {
    ...
    static function nombreDia ($dia = "", $lang = "es") {
        ...
        if ($dia != 0 and empty($dia))
            $dia = date('w');
        if (!array_key_exists($lang, self::$diasSemana))
            throw new Exception("No existe traducción de días para el lenguaje $lang", 1);
    ...
    static function fechaInvertida ($fecha = '', $parametro = '-') {
        ...
        if ($fecha == '') {
            ...
            if ($fechaTipoFormato [4] == '-' || $fechaTipoFormato [4] == '/') {
                ...
                return date('d' . $fechaTipoFormato [4] . 'm' . $fechaTipoFormato [4] . 'Y',
                            strtotime($fechaTipoFormato));
            }
            else if ($fechaTipoFormato [2] == '-' || $fechaTipoFormato [2] == '/') {

                return date('Y' . $fechaTipoFormato [2] . 'm' . $fechaTipoFormato [2] . 'd',
                            strtotime($fechaTipoFormato));
            }
        }
    }
