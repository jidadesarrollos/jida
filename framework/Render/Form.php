...
class Form extends \Selector {
    ...
    private function _cargarFormulario ($form) {
        ...
        if (\Directorios::validar(DIR_APP . DS . 'formularios/' . strtolower($form) . '.json')) {

            $this->_path = DIR_APP . DS . 'formularios/' . $form . '.json';
        }
        else if (\Directorios::validar(DIR_FRAMEWORK . DS . 'formularios/' . $form . '.json')) {
            $this->_path = DIR_FRAMEWORK . DS . 'formularios/' . $form . '.json';
        }
        else {
    ...
    private function validarJson () {
        ...
        if (json_last_error() != JSON_ERROR_NONE) {
            throw new Excepcion("El formulario  " . $this->_path . " no esta estructurado correctamente",
                                $this->_ce . "0");
        }
    ...
    private function _procesarEstructura () {
        ...
        for ($i = 0; $i <= count($estructura) - 1; ++$i):
            ...
            if (count(explode(",", $distribucion)) < $columnas) {
                throw new Excepcion("La estructura no esta armada correctamente. La distribución es menor a la cantidad de columnas" . $distribucion,
                                    $this->_ce . '1');
        ...
        if (count($this->_estructura) > $this->_totalCampos) {
            throw new Excepcion("La estructura tiene mayor cantidad de campos que el formulario " . $this->_configuracion->nombre,
                                $this->_ce . '5');
        }
    ...
    private function _instanciarCamposConfiguracion () {
        ...
        if ($this->_totalCampos < 1) {
            throw new Excepcion("El formulario " . $this->_formulario . " no tiene campos registrados",
                                $this->_ce . "1");
