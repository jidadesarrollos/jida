                ...
                <?php foreach ($data->areas as $key => $area): ?>
                    <section class="col-md-4 col-sm-6 col-xs-12">
                        <article class="area-elemento-container" id="area-<?= $area['id'] ?>">
                            <h3 class="area-nombre" data-toggle="collapse" data-target="#body-<?= $area['id'] ?>"
                                aria-expanded="false" aria-controls="body-<?= $area['id'] ?>">
                                <?= $area['nombre'] ?>
                            </h3>
                            <section class="area-body" id="body-<?= $area['id'] ?>">
                                ...
                                <?php endif ?>
                                <?php
                                if (array_key_exists($area['id'],
                                                     $elementosCargados) and count($elementosCargados[$area['id']] > 0)):
                                    $i = 1;
                                    ?>
                            ...
                            <div class="seccion-btns">
                                <div class="row">
                                    <div class="col-md-12 text-right">
                                        ...
                                        <button data-area="<?= $area['id'] ?>" class="btn-primary btn"
                                                data-jadmin="elementos">Agregar Elemento
                                        </button>
                                    </div>
                                </div>
                            </div>
                ...
                <?php foreach ($data->elementos as $key => $elemento): ?>
                    ...
                    <div class="col-md-3">
                        <div class="seccion-elemento">
                            ...
                            <div class="elemento-form hidden"
                                 data-elemento='{"nombre":"<?= $ele->nombre() ?>","id":"<?= $ele->id() ?>"}'>
                                <?= $ele->jform() ?>
                            </div>
                        </div>
