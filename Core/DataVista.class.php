...
class DataVista {
     ...
     * @param boolean $dir    TRUE si se desea usar la constante URL_JS como path de ubicación
     * @param         boolean footer True Define si el js se imprimirá arriba o abajo
     */
    function addJs($js, $dir = TRUE, $ambito = "", $footer = TRUE) {
        if ($dir === TRUE) {
            $dir = URL_JS;
        }
        if (is_array($js)) {
    ...
    function addJsModulo($js, $ruta = TRUE) {
        ...
        if (is_array($js)) {
            foreach ($js as $key => $archivo) {
                ...
                if ($ruta)
                    $this->js[] = "/" . $modulo . "/htdocs/js/" . $archivo; else $this->js[] = $archivo;
            }
        } elseif (is_string($js)) {
            if ($ruta)
                $this->js[] = "/" . $modulo . "/htdocs/js/" . $js; else $this->js[] = $js;
        }
    ...
    function addCssModulo($css, $ruta = TRUE) {
        ...
        if (is_array($css)) {
            foreach ($css as $key => $archivo) {
                ...
                if ($ruta)
                    $this->css[] = "/" . $modulo . "/htdocs/css/" . $css; else $this->css[] = $archivo;
            }
        } elseif (is_string($css)) {
            if ($ruta)
                $this->css[] = "/" . $modulo . "/htdocs/css/" . $css; else $this->css[] = $css;
        }
    ...
    function addJsAjax($js, $dir = TRUE, $ambito = "") {
        ...
        if (is_array($js)) {
            foreach ($js as $key => $archivo) {
                if (!empty($ambito))
                    $this->jsAjax[ $ambito ][] = $dir . $archivo; else
                    $this->jsAjax[] = $dir . $archivo;
            }
        } else {
            if (!empty($ambito))
                $this->js[ $ambito ][] = $dir . $js; else
                $this->jsAjax[] = $dir . $js;
        }
    }
    ...
    function addCSS($css, $constante = URL_CSS, $ambito = "") {
        ...
        if (is_array($css)) {
            ...
            foreach ($css as $key => $value) {
                if (!empty($ambito))
                    $this->css[ $ambito ][] = $constante . $value; else
                    $this->css[] = $constante . $value;
            }
        } else {
            array_push($this->cssAgregado, $css);
            if (!empty($ambito))
                $this->css[ $ambito ][] = $constante . $css; else {
    ...
    function addCodeJs($arg1, $file = FALSE) {
        if ($file == TRUE)
            $this->js['code'][] = ['archivo' => $arg1]; else {
