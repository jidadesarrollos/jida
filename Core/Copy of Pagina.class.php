...
//
	...
	function imprimirLibrerias($lang,$modulo=""){
		...
		if(array_key_exists(ENTORNO_APP, $data)){
			...
			//Se eliminan
			//if($lang=='js') Debug::mostrarArray($dataInclude,0);
			foreach ($dataInclude as $key => $value) {
				
				if(is_array($value) and $key!=$modulo){
					if(array_key_exists('tipo', $value) and $value['tipo']=='script'){
						continue;
					}else
		...
		$libsHTML = "";
		$cont=0;
		// Debug::mostrarArray($librerias,0);
		// if($lang=='js')
			// Debug::mostrarArray($this->data->jsAgregado,0);
		// else Debug::mostrarArray($this->data->cssAgregado,0);
		$librerias = ($lang=='js')?array_merge($librerias,$this->data->jsAgregado):array_merge($librerias,$this->data->cssAgregado);
		// Debug::mostrarArray($librerias,0);
		...
		switch ($lang) {
			case 'js':
				...
				if(is_array($libreria)){
					if(!array_key_exists('attrs', $libreria)) throw new Exception("No se ha pasado la libreria js correctamente", 400);
					
					$html = Selector::crear('script',$libreria['attrs'],null,$cont);
				}else 
